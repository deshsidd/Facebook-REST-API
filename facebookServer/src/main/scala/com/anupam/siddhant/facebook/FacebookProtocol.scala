...
case class Image(imagedata:String,caption:String,id:Int, token : LoginData)
  ...
  case class ShareKey(ctx:RequestContext, map : AESKeyMap)
  ...
  var Users = Vector(User(1000000,"siddhant","siddhantd28","sid"), User(2000000,"anuj","anuj","@nuj") )

  var Groups= Vector[Group]()

  var AllUsers = scala.collection.mutable.Map(1 -> "siddhant" , 2 -> "anuj")

  var actorMap = scala.collection.mutable.Map[String, ActorRef]()
  ...
  object AESKeyMap extends DefaultJsonProtocol{
   ...
   implicit val aesformat = jsonFormat2(AESKeyMap.apply)
  }

  object KeyAndData extends DefaultJsonProtocol{

   implicit val keyformat = jsonFormat4(KeyAndData.apply)
  }
